...
t_span = (0, 4); t_eval = np.linspace(0, 4, 100)
...
def dynamics_core(t, state, forces=None):
    ...
    for i in range(n):
        ...
        Hi = Ri @ hat(Xi)
        derivatives[idx:idx+9] = Hi.flatten()
        derivatives[idx+9:idx+12] = Vi
        derivatives[idx+12:idx+15] = Ai
        derivatives[idx+15:idx+18] = np.zeros(3)
    return derivatives
...
R3_0 = np.array([np.sqrt(2)/2, -np.sqrt(2)/2, 0,
                 ...
                 0, 0, 1])
X1_0 = np.array([0, 0.24, 0.25])
X2_0 = np.array([0, 0.2, -0.25])
X3_0 = np.array([0, 0.2, 0.02])
